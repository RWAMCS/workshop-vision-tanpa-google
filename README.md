...
Clone repository (atau lewati langkah ini jika folder project sudah ada di komputer):

```powershell
git clone https://github.com/RWAMCS/workshop-vision-tanpa-google.git

cd workshop-vision-tanpa-google
```

> Catatan: jika folder project Anda berada di lokasi lain, sesuaikan perintah `cd` di atas
> (contoh: `cd "D:\kuliah\workshop-vision-tanpa-google"`).
...
Pastikan terminal berada di folder project (baris `cd` boleh dilewati jika sudah di dalamnya):
...
Percobaan 2 = aplikasi web Flask (`app.py`). Pastikan terminal berada di folder project, lalu langkah pengujian:
